...
class FoodControllerTest {
    ...
    void findByQuery_whenFoodExist_shouldReturn200AndDto() throws Exception {
        ...
        PagedResponse<FoodResponseDto> pagedResponse = new PagedResponse<>(List.of(dto), pagination);
    ...
    void findByQuery_whenFoodDoesNotExist_shouldReturnNoContentAndEmptyList() throws Exception {
        // Given
    ...
    void findByQuery_whenOffsetIsNegative_shouldReturnBadRequestAndErrorResponse() throws Exception {
        // Given
        ApiResponse<List<FoodResponseDto>> pagedResponse = ApiResponse.error(
                new ApiError("validation", "findByQuery.offset must be greater than or equal to 0"));
        String expected = objectMapper.writeValueAsString(pagedResponse);
    ...
    void findByQuery_whenLimitIsNegative_shouldReturnBadRequestAndErrorResponse() throws Exception {
        // Given
        ApiResponse<List<FoodResponseDto>> pagedResponse = ApiResponse.error(
    ...
    @Test
    @DisplayName("When query is valid and suggestions exist, should return 200 OK and suggestion list")
    void getSearchSuggestions_whenSuggestionsExist_shouldReturnOkAndList() throws Exception {
        // Given
    ...
    void getSearchSuggestions_whenNoSuggestions_shouldReturnNoContent() throws Exception {
        // Given
        when(foodService.getSearchSuggestions("xyz")).thenReturn(Collections.emptyList());
    ...
    void getSearchSuggestions_whenQueryMissing_shouldReturnBadRequest() throws Exception {
        mockMvc.perform(get("/api/foods/search-suggestions"))
                .andExpect(status().isBadRequest());
    }
    ...
    void patch_whenSuccessful_shouldReturn200AndUpdatedDto() throws Exception {
        ...
        when(foodService.patch(eq(id), any(FoodPatchRequestDto.class))).thenReturn(foodResponseDto);
    ...
    @Test
    @DisplayName("When patch request has validation errors, should return 400 Bad Request with error")
    void patch_whenValidationError_shouldReturnBadRequest() throws Exception {
        ...
        ApiResponse<Object> response = ApiResponse.error(
                new ApiError("nutriments.carbohydrates", "must be greater than or equal to 0.0"));
        String expected = objectMapper.writeValueAsString(response);
